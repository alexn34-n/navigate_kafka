  ...
  <div style="width: 100%; min-height: 100vh; display: inline-flex;">
    <div class="messages">
      <div v-for="(message, ind) in messages" :key="ind" :title="JSON.stringify(message)">
        <div class="msg" v-if="message.source === 'PORT'">A</div>
    ...
    <div class="radar">
      ...
      <div class="planeArea">
        <Ship v-for="(row, ind) in boards" :key="ind" :level="ind" :ship="row"></Ship>
      </div>
...
import Plane from "../components/Ship";
export default {
  components: {Plane},
  data() {
    return {
...
.radar {
  width: 90%;
  position: relative;
  background: transparent url("assets/sea.png");
  min-height: 100vh;
}
.msg{
  border-radius: 10px;
